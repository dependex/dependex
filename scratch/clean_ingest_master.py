...
def parse_item(item):
    ...
    # If street is C/o and city has Via/Piazza...
    ...
    # Check if prov has 5-digit CAP and real city name
    ...
    # Resolve Province, Region, Lat, Lon from CAP
    prov_code = ""
    lat = 42.5000
    lon = 12.5000
    ...
    if cap and len(cap) >= 2 and cap[:2] in CAP2_MAP:
        ...
        lat = cap_lat
        lon = cap_lon
        if not reg or reg in ['Unknown', 'Italia', '']:
            reg = cap_reg
        if not city:
            city = cap_city

    # Normalize Region names
    reg_clean = reg.replace('Friuli Venezia Giulia', 'Friuli-Venezia Giulia').replace('Emilia Romagna', 'Emilia-Romagna').replace('Trentino Alto Adige', 'Trentino-Alto Adige')
    if not reg_clean or reg_clean in ['Unknown', '']:
        reg_clean = 'Italia'

    if not city:
        city = prov or reg_clean or 'Italia'
    ...
    def_email, def_phone, def_parent = REGIONAL_DEFAULTS.get(reg_clean, ('info@aicat.net', '800 974250', 'AICAT Nazionale'))
...
def main():
    ...
    # Rollback previous flawed insert if present
    cur.execute("DELETE FROM cat_clubs_italy WHERE id >= 891")
    cur.execute("DELETE FROM crm_club_contacts WHERE id >= 891")
    cur.execute("DELETE FROM dependex_world_registry WHERE id >= 1194")
    conn.commit()
    print("Database cleaned to verified base (890 records in cat_clubs_italy).")
    ...
    for item in data:
        ...
        # Filter for Hudolin / CAT / ACAT / Addiction Recovery groups
        ...
        # 3. Insert dependex_world_registry
        cur.execute("""
