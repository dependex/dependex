...
cur.execute("SELECT id, sic_id, entity_name, city, address, cap, province FROM cat_clubs_italy WHERE region = 'Italia'")
rows = cur.fetchall()
print(f"Total Italia rows to resolve: {len(rows)}")

resolved = 0
for row_id, sic_id, name, city, address, cap, prov in rows:
    found_cap = cap
    if not found_cap:
        for fld in [address, city, prov, name]:
            m = re.search(r'\b(\d{5})\b', fld or '')
            if m:
                found_cap = m.group(1)
                break

    if found_cap and found_cap[:2] in CAP2_MAP:
        cap_city, cap_prov, cap_reg, cap_lat, cap_lon = CAP2_MAP[found_cap[:2]]
        cur.execute("""
            UPDATE cat_clubs_italy
            SET region = ?, province = ?, latitude = ?, longitude = ?, cap = ?
            WHERE id = ?
        """, (cap_reg, cap_prov, cap_lat, cap_lon, found_cap, row_id))

        cur.execute("""
            UPDATE crm_club_contacts
            SET region = ?, province = ?, cap = ?
            WHERE sic_id = ?
        """, (cap_reg, cap_prov, found_cap, sic_id))

        cur.execute("""
            UPDATE dependex_world_registry
            SET region = ?, province = ?, latitude = ?, longitude = ?
            WHERE sic_id = ?
        """, (cap_reg, cap_prov, cap_lat, cap_lon, sic_id))

        resolved += 1

conn.commit()
print(f"Successfully resolved by CAP: {resolved} / {len(rows)}")

print("\nUpdated Regional Distribution in cat_clubs_italy:")
